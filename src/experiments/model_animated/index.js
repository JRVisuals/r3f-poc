...
const Model = ({fileRef}) => {
  ...
  useEffect(() => void mixer.clipAction(modelAnimationClips, modelRef.current).play(), [mixer, modelAnimationClips])
  ...
  return (
      ...
      <primitive ref={modelRef} object={gltf.scene} scale={[25,25,25]} position={[0,-0.25,0]} castShadow receiveShadow/>    
...
const Base = () => {
  ...
  return (
    <mesh ref={baseRef} position={[0,-0.5,0]}>
      <cylinderBufferGeometry attach="geometry" args={[6, 6, .5, 128, 8]} />
      <meshStandardMaterial attach="material"  color="#552200" shininess={0} receiveShadow/>
    </mesh>
  )
...
export default {
    ...
    metadata:{
      name: 'Model(ANIMATED)',
      author: '',
      description: 'Loads (large) animated dragon using React.Suspense and external .glb files. Model credit: elly77ellison on sketchfab',
    },
};
